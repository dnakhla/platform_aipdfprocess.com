import os
import pytest
import shutil
import uuid
import pikepdf
from unittest.mock import MagicMock, patch

# Set dummy env vars for the worker import
os.environ["AWS_ACCESS_KEY_ID"] = "test"
os.environ["AWS_SECRET_ACCESS_KEY"] = "test"
os.environ["AWS_DEFAULT_REGION"] = "us-east-1"
os.environ["AWS_ENDPOINT_URL"] = "http://localhost:4566"
os.environ["INPUT_BUCKET"] = "aipdf-input-dev"
os.environ["ARTIFACTS_BUCKET"] = "aipdf-artifacts-dev"

@pytest.fixture
def mock_s3(mocker):
    mock = mocker.patch("app.s3_client")
    return mock

@pytest.fixture
def test_pdf():
    path = "/tmp/test_opt.pdf"
    doc = pikepdf.new()
    doc.add_blank_page()
    doc.save(path)
    doc.close()
    yield path
    if os.path.exists(path):
        os.remove(path)

def test_handler_compress(mock_s3, test_pdf):
    from app import handler
    
    mock_s3.download_file.side_effect = lambda bucket, key, dest: shutil.copy(test_pdf, dest)
    
    event = {
        "jobId": "test-job-opt-1",
        "inputKey": "test.pdf",
        "operation": {
            "type": "compress",
            "params": {}
        }
    }
    
    res = handler(event, None)
    
    assert res["success"] is True
    assert "outputKey" in res
    assert mock_s3.upload_file.called
    assert res["metadata"]["optimized"] is True

def test_handler_sanitize(mock_s3, test_pdf):
    from app import handler
    
    mock_s3.download_file.side_effect = lambda bucket, key, dest: shutil.copy(test_pdf, dest)
    
    event = {
        "jobId": "test-job-opt-2",
        "inputKey": "test.pdf",
        "operation": {
            "type": "sanitize",
            "params": {}
        }
    }
    
    res = handler(event, None)
    
    assert res["success"] is True

def test_handler_linearize(mock_s3, test_pdf):
    from app import handler
    
    mock_s3.download_file.side_effect = lambda bucket, key, dest: shutil.copy(test_pdf, dest)
    
    event = {
        "jobId": "test-job-opt-3",
        "inputKey": "test.pdf",
        "operation": {
            "type": "linearize",
            "params": {}
        }
    }
    
    res = handler(event, None)
    
    assert res["success"] is True
