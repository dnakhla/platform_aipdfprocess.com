...
def setup_worker():
    ...
    env = os.environ.copy()
    env["PYTHONPATH"] = "/home/node/.pennybot/workspace/projects/aipdf-platform/services/worker-structural"
    proc = subprocess.Popen(cmd, env=env, cwd="/home/node/.pennybot/workspace/projects/aipdf-platform/services/worker-structural")
    # Wait for startup
    time.sleep(3)
    return proc
...
def test_structural_integration():
    # 1. Create a test PDF
    pdf_path = "/tmp/test_integration_input.pdf"
    doc = fitz.open()
    doc.new_page().insert_text((50,50), "Integration Test Page 1")
    doc.new_page().insert_text((50,50), "Integration Test Page 2")
    doc.save(pdf_path)
    doc.close()
    
    # 2. Upload to S3
    input_key = "test_integration.pdf"
    s3.upload_file(pdf_path, "aipdf-input-dev", input_key)
    print(f"Uploaded input to {input_key}")
    
    # 3. Call worker for rotate
    ...
    local_output = "/tmp/test_integration_output.pdf"
    s3.download_file("aipdf-artifacts-dev", output_key, local_output)
    
    doc_res = fitz.open(local_output)
    assert len(doc_res) == 2
    # Check if rotated (optional, but good)
    assert doc_res[0].rotation == 90
    doc_res.close()
